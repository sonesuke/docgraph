...
- **CI Pipeline**: The CI workflow will use the `devcontainers/ci` action to build and run tests _inside_ the exact same
  container image defined in `.devcontainer/devcontainer.json`.
...
We discovered discrepancies between the Continuous Integration (CI) environment (Ubuntu-latest) and the local
development environment (Dev Container / Debian Bullseye). Specifically, the Node.js version differed (v18 vs latest
LTS), creating a risk of "works on my machine, fails in CI" scenarios.
