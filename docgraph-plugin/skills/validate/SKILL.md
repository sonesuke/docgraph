...
> [!IMPORTANT]
> This skill must perform checks **strictly and rigorously**. Accuracy in ID naming, responsibility scope, and template adherence is critical for the integrity of the documentation graph. **Even cosmetic inconsistencies must be flagged.**
...
1. **Formatting**: Run `npm run format:md -- --check` to verify Markdown style.
...
- **Level**: STRICT
- **Failure Policy**: FAIL immediately.
- **Rule of Thumb**: Generally, nodes with the same prefix should be grouped in the same file or a specific directory structure. **This is not an exception mechanism.**
