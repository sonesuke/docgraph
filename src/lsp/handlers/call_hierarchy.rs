...
pub async fn prepare_call_hierarchy(
    ...
    if let Ok(path) = uri.to_file_path() {
        ...
        // Delegate to Core
        if let Some(target_id) = crate::core::locate::locate_id_at_position(&blocks, &[], &path, line, col)
    ...
    for loc in locations {
        ...
        if let Some(source_block) = blocks.iter().find(|b| 
            ...
            loc.range_end_line <= b.line_end
             ...
             let is_edge = source_block.edges.iter().any(|e| 
                e.line == loc.range_start_line && 
                e.col_start == loc.range_start_col
             );

             if is_edge && let Ok(u) = Url::from_file_path(&source_block.file_path) {
                    calls.push(CallHierarchyIncomingCall {
                        from: CallHierarchyItem {
                            name: source_block.name.clone().unwrap_or_else(|| source_block.id.clone()),
                            kind: SymbolKind::INTERFACE,
                            tags: None,
                            detail: None,
                            uri: u,
                            range: Range {
                                start: Position { line: source_block.line_start as u32 - 1, character: 0 },
                                end: Position { line: source_block.line_start as u32 - 1, character: 0 },
                            },
                            selection_range: Range {
                                start: Position { line: source_block.line_start as u32 - 1, character: 0 },
                                end: Position { line: source_block.line_start as u32 - 1, character: 0 },
