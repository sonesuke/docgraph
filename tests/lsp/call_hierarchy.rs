...
Target
    ...
    let prepare_res: Value = c.send_request("textDocument/prepareCallHierarchy", json!({
        "textDocument": {"uri": uri_b},
        "position": {"line": 1, "character": 8}
    })).await?;

    let items = prepare_res.get("result").and_then(|r| r.as_array()).expect("Prepare call hierarchy failed");
    let item = &items[0];

    // Incoming calls
    let incoming_res: Value = c.send_request("callHierarchy/incomingCalls", json!({
        "item": item
    })).await?;

    let calls = incoming_res.get("result").and_then(|r| r.as_array()).expect("Incoming calls failed");
    assert!(!calls.is_empty(), "Should have incoming calls");
    
    let from_a = calls.iter().any(|call| {
        call.get("from").and_then(|f| f.get("name")).and_then(|n| n.as_str()).unwrap_or("") == "A"
    });
