...
References: [REQ-001](a.md#REQ-001)
    ...
    // 4. Definition Test
    let def_res: Value = c.send_request("textDocument/definition", json!({
    ...
    let loc = if res.is_array() {
        res.as_array().unwrap().get(0).expect("Empty definition result")
    } else {
        res
    };
    
    let uri = loc.get("uri").and_then(|u| u.as_str()).expect("No URI in definition");
    assert!(uri.ends_with("a.md"), "Definition should point to a.md");

    // 5. References Test
    let ref_res: Value = c.send_request("textDocument/references", json!({
        "textDocument": {"uri": file_a_uri},
        "position": {"line": 1, "character": 8},
        "context": { "includeDeclaration": true }
    })).await?;
    
    let refs = ref_res.get("result").and_then(|r| r.as_array()).expect("References result should be array");
    
    assert!(refs.len() >= 2, "Should find at least 2 references (def + ref)");
    let found_b = refs.iter().any(|r| {
        r.get("uri").and_then(|u| u.as_str()).unwrap_or("").ends_with("b.md")
    });
