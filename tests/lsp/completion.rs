...
[REQ
    ...
    let comp_res: Value = c.send_request("textDocument/completion", json!({
        "textDocument": {"uri": file_uri},
        "position": {"line": 4, "character": 4}
    })).await?;

    let items = comp_res.get("result").and_then(|r| r.get("items")).or_else(|| comp_res.get("result")).and_then(|r| r.as_array()).expect("Completion items not found");
    
    let found = items.iter().any(|item| {
        item.get("label").and_then(|l| l.as_str()).unwrap_or("") == "REQ-001"
    });
